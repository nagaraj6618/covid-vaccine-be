...
async function getAllAppointment(req, res) {
   ...
   try {
      ...
      if (!status && !date) {
         ...
         // Fetch patient data for each appointment and merge the details
         const getPatientDataByAppointment = await Promise.all(
            getAllAppointmentData.map(async (appointment) => {
               const patientData = await patientModel.findById(appointment.patientId);
               return {
                  ...appointment.toObject(),  
                  patientDetails: patientData ? patientData.toObject() : null 
               };
            })
         );

         // console.log(getPatientDataByAppointment);
         ...
         // const getPatientDataByAppointment  = await Promise.all(
         //    getAllAppointmentData.map(
         //       async(prev) =>  await patientModel.findById(prev.patientId)
         //    ));
         // console.log(getPatientDataByAppointment)
         return res.status(200).json(
            {
               ...
               data: getPatientDataByAppointment
            });
      }
      else if (status && !date) {
